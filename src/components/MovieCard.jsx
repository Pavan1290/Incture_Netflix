...
export default function MovieCard({ movie, showActions = false }) {
  ...
  const openTrailer = (e) => { e.stopPropagation(); window.open(getTrailerUrl(movie), '_blank', 'noopener') }
  return (
  ...
  <Poster movie={movie} className="card-img-top" style={{ aspectRatio: '2 / 3', objectFit: 'cover' }} />
      <div className="card-body d-flex flex-column">
        ...
        <div className="mt-auto d-flex gap-2">
          <button className="btn btn-outline-light btn-sm" onClick={openTrailer}>Watch Trailer</button>
          {showActions && (
